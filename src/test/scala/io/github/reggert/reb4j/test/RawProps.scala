...
{
	@Test def toPattern : Unit = check(toPattern(arbitrary[Raw]), minSuccessful(100000), maxDiscarded(500000))
}
...
{
	@Test def toPattern : Unit = check(toPattern(arbitrary[Raw.Compound]), minSuccessful(100000), maxDiscarded(500000))
}
...
{
	@Test def toPattern : Unit = check(toPattern(arbitrary[Entity]), minSuccessful(100000), maxDiscarded(500000))
}
...
{
	@Test def toPattern : Unit = check(toPattern(arbitrary[EscapedLiteral]), minSuccessful(100000), maxDiscarded(500000))
}